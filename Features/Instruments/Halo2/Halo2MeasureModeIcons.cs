...
/// <summary>
/// Glyph kinds drawn by <see cref="HaloMeasureModeIconDrawable"/> on the Halo 2 mode-segment toolbar.
/// Custom-drawn (vs. PNGs) so the stroke color tracks the current theme without re-importing assets.
/// </summary>
public enum HaloMeasureModeIconKind { Table, Chart, Calibration, Settings }

/// <summary>
/// Custom <see cref="IDrawable"/> for Halo 2 toolbar icons.
///
/// <paramref name="getStroke"/> is a lambda (not a captured <see cref="Color"/>) so the icon can
/// re-fetch the latest theme color on every redraw — important for live theme switching where
/// the Drawable instance is reused but the color must change.
/// Stroke thickness scales with the rect width so the icon stays crisp at any size.
/// </summary>
...
/// <summary>Factory for the GraphicsView host of a Halo 2 mode icon.</summary>
